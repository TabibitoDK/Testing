...
const App = () => {
  const [apps, setApps] = useState([
    { id: 'fileManager', name: 'File Manager', icon: fileManagerIcon, isOpen: false, isMinimized: false },
      ...
      <Desktop>
        ...
        {apps.map((app) => {
          if (app.isOpen && !app.isMinimized) {
            switch (app.id) {
              case 'fileManager':
                return <FileManager key={app.id} onClose={() => handleCloseApp(app.id)} onMinimize={() => handleMinimizeApp(app.id)} />;
              case 'terminal':
